...
function fecharModalPixAlert() {
  document.getElementById('modal-pix-alert').style.display = 'none';
    ...
    if (srcImagem.startsWith("data:image")) {
        uploadImgur(srcImagem).then(urlImgur => {
          ...
          adicionarPresenteVisual(nomeDoador, nomePresente, comentario, urlImgur); // <-- TROQUE A ORDEM AQUI
          abrirModalPresenteConfirmado();
        });
    } else {
        salvarPresenteGoogleForms(nomeDoador, nomePresente, comentario, srcImagem);
        adicionarPresenteVisual(nomeDoador, nomePresente, comentario, srcImagem); // <-- TROQUE A ORDEM AQUI
        abrirModalPresenteConfirmado();
    }
  ...
  const lista = document.getElementById("lista-confirmados");
  const div = document.createElement("div");
  let url = imagemUrl && imagemUrl.trim() ? imagemUrl.trim() : "teste.png";
  const comentarioGrande = comentario && comentario.length > 120;
  div.className = "presente" + (comentarioGrande ? " presente--imagem-esquerda" : "");
  div.innerHTML = `
    <img src="${url}" alt="${nomePresente}" class="presente-imagem" onerror="this.src='teste.png'">
    <div class="presente-info">
      <p><strong>${nomePresente}</strong></p>
      <p><strong>De:</strong> <em>${nomeDoador}</em></p>
...
function carregarPresentesConfirmados() {
  ...
  fetch(urlCSV)
    ...
    .then(csv => {
      const linhas = splitCSVLinhas(csv).filter(l => l.trim()).slice(1); // pula o cabeçalho
